...
export async function enforceJoinLimit(bot, repos, config, user) {
    const joins = repos.incrementJoinCount(user.id);
    if (user.is_banned)
        return;
    if (joins < config.maxJoinsBeforeBan)
        return;
    ...
    await safeSendMessage(bot, config.adminChatId, `🔒 <b>Авто-бан по лимиту входов</b>\nПользователь <code>${user.telegram_id}</code> заходил в основной чат через бота ${joins} раз и больше не может подавать анкеты и брони.\nСнять ограничение: <code>/unban ${user.telegram_id}</code>`, { parse_mode: "HTML" });
}
