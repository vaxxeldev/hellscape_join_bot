...
export function missingSubscriptionsMessage(check) {
    ...
    if (!check.life)
        lines.push(`• ${pe(premiumEmoji.announcement, "📣")} лайф-канал`);
    if (!check.info)
        lines.push(`• ${pe(premiumEmoji.info, "ℹ")} инфо-канал`);
...
export function recruitmentClosedMessage(memberCount, memberLimit) {
...
export function applicationCard(app, user, subscriptions, previousCount) {
...
export function reservationCard(reservation, user, subscriptions) {
    const reserveUntil = reservation.reservation_kind === "waitlist"
...
export function rulesMessage(config) {
...
export function applicationStatusMessage(app) {
...
export function activeReservationMessage(reservation) {
    return `<b>Бронь #${reservation.id}</b>\nРоль: ${escapeHtml(reservation.role_name)}\nСтатус: <code>${reservationStatusLabel(reservation.status)}</code>\nДо: ${escapeHtml(formatDate(reservation.reserve_until))}`;
}
export function adminPanelMessage(stats) {
...
export function applicationsListMessage(apps) {
    return apps
        .map((app) => `#${app.id} · <code>${applicationStatusHtml(app.status, app.reject_reason)}</code> · пользователь <code>${app.user_id}</code> · ${escapeHtml(app.role)} · ${escapeHtml(formatDate(app.created_at))}`)
        .join("\n");
}
...
export function banUsageMessage(isBanned) {
    return isBanned
...
export function banResultMessage(user, isBanned) {
...
export function reservationsListMessage(reservations) {
    return reservations
        .map((item) => `#${item.id} · <code>${reservationStatusLabel(item.status)}</code> · ${escapeHtml(item.role_name)} · до ${escapeHtml(formatDate(item.reserve_until))} · пользователь <code>${item.user_id}</code>`)
        .join("\n");
}
export function changeReservationUsageMessage(status) {
...
export function reservationStatusChangedMessage(id, status) {
...
export function activeApplicationExistsMessage(applicationId) {
...
export function usernameStepMessage(step, defaultUsername) {
