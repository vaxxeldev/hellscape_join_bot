...
export class DeveloperHandlers {
  ...
  async handleMessage(ctx: BotContext) {
    if (!this.isDeveloper(ctx) || ctx.chat?.type !== "private" || !ctx.from) return false;
    const state = this.repos.getState(ctx.from.id);
    if (state?.flow !== "developer_broadcast") return false;

    const message = ctx.message as MessageLike | undefined;
  ...
  private async openPanel(ctx: BotContext) {
    if (!this.isDeveloper(ctx)) return;
  ...
  private async backToPanel(ctx: BotContext) {
    if (!(await this.ensureDeveloperCallback(ctx))) return;
    if (ctx.from) this.repos.clearState(ctx.from.id);
  ...
  private async startBroadcastCommand(ctx: BotContext) {
    if (!this.isDeveloper(ctx)) return;
    ...
    } satisfies BroadcastData);
  }

  private async startBroadcastCallback(ctx: BotContext) {
    if (!(await this.ensureDeveloperCallback(ctx))) return;
    const messageId = this.callbackMessageId(ctx);
    if (!ctx.from || !ctx.chat || !messageId) return;
  ...
  private async receiveBroadcastText(ctx: BotContext, message: MessageLike, data: BroadcastData) {
  ...
  private async receiveBroadcastMedia(ctx: BotContext, message: MessageLike, data: BroadcastData) {
  ...
  private async askBroadcastButton(chatId: number, telegramId: number, data: BroadcastData) {
  ...
  private async askBroadcastButtonText(ctx: BotContext) {
    if (!(await this.ensureDeveloperCallback(ctx))) return;
    const state = this.getBroadcastState(ctx);
  ...
  private async receiveBroadcastButtonText(ctx: BotContext, message: MessageLike, data: BroadcastData) {
  ...
  private async receiveBroadcastButtonUrl(ctx: BotContext, message: MessageLike, data: BroadcastData) {
  ...
  private async showBroadcastPreview(chatId: number, telegramId: number, data: BroadcastData) {
  ...
  private async cancelBroadcastPreview(ctx: BotContext) {
    if (!(await this.ensureDeveloperCallback(ctx))) return;
    const state = this.getBroadcastState(ctx);
    const data = state ? this.parseBroadcastData(state.data) : {};
    if (ctx.chat) await this.cleanupBroadcastPreview(ctx.chat.id, data);
    if (ctx.from) this.repos.clearState(ctx.from.id);
  ...
  private async wipeFromPanel(ctx: BotContext) {
    if (!(await this.ensureDeveloperCallback(ctx))) return;
  ...
  private async trackBotChat(ctx: BotContext) {
  ...
  private async editControlMessage(chatId: number, messageId: number | undefined, text: string, extra: Record<string, unknown>) {
    if (!messageId) {
  ...
  private async cleanupBroadcastPreview(chatId: number, data: BroadcastData) {
  ...
  private async tryDeleteMessage(chatId: number, messageId: number | undefined) {
    if (!messageId) return;
    try {
  ...
  private getBroadcastState(ctx: BotContext) {
    if (!ctx.from) return null;
    const state = this.repos.getState(ctx.from.id);
    return state?.flow === "developer_broadcast" ? state : null;
  }

  private parseBroadcastData(raw: string) {
    try {
...
function developerStatsMessage(
  stats: ReturnType<Repositories["developerStats"]>,
  roleCatalog: ReturnType<RoleService["catalogStats"]>,
  config: AppConfig,
) {
...
function broadcastStartedMessage(total: number) {
...
function broadcastFinishedMessage(success: number, failed: number) {
...
function broadcastReplyMarkup(data: BroadcastData) {
  if (!data.buttonUrl || (!data.buttonText && !data.buttonEmojiId)) return {};
  const button: Record<string, string> = {
    text: data.buttonText || "·",
    url: data.buttonUrl,
  };
  if (data.buttonEmojiId) button.icon_custom_emoji_id = data.buttonEmojiId;
  return {
    reply_markup: {
...
function parseButtonText(text: string, entities: MessageEntityLike[]) {
...
function lastPhotoFileId(message: MessageLike) {
