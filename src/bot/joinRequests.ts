...
export class JoinRequestHandlers {
  ...
  private async handle(ctx: BotContext) {
    ...
    if (invite.application_id) {
      ...
      else if (app.status !== "approved") reasons.push(`анкета не одобрена: ${applicationStatusLabel(app.status, app.reject_reason)}`);
    } else if (invite.reservation_id) {
      if (!reservation) reasons.push("связанная бронь не найдена");
      else if (reservation.status !== "approved") reasons.push(`бронь не активна: ${reservation.status}`);
    } else {
  ...
  private async rejectJoinRequest(
    invite: { id: number; invite_link: string; application_id: number | null; reservation_id: number | null },
    userId: number,
    from: { id: number; username?: string },
    reasons: string[],
    applicationId: number | null = invite.application_id,
    reservationId: number | null = invite.reservation_id,
  ) {
  ...
  private async handleNonActiveOwnerRequest(inviteId: number, from: { id: number; username?: string }) {
    ...
    if (this.getConfig().autoDeclineInvalidJoinRequests) await this.declineTelegramJoinRequest(from.id);
    await safeSendMessage(
      this.bot,
      from.id,
      "Твоя персональная ссылка больше недействительна (отозвана или истёк срок). Подай новую анкету через меню бота — после одобрения придёт свежая ссылка.",
    );
    await safeSendMessage(
      this.bot,
      this.getConfig().adminChatId,
      `${pe(premiumEmoji.notification, "⚠")} <b>Заход по недействительной ссылке</b>\n\nВладелец: <code>${from.id}</code> ${escapeHtml(
        from.username ? `@${from.username}` : "no_username",
      )}\nСтатус ссылки: ${inviteLinkStatusLabel(current.status)}. Пользователю предложено подать заявку заново.`,
      { parse_mode: "HTML" },
    );
  ...
  private async banLeakedLinkOwner(ownerUserId: number) {
    const owner = this.repos.getUserById(ownerUserId);
    if (!owner || owner.is_banned) return;
