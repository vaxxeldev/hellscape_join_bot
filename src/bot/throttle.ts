...
export function createThrottle(getConfig: () => AppConfig): MiddlewareFn<BotContext> {
  ...
  const sweep = setInterval(() => {
    ...
    for (const [userId, entry] of hits) {
      ...
      if (!entry.timestamps.length && entry.lastNotifiedAt <= cutoff) hits.delete(userId);
    }
  }, 60_000);
  sweep.unref?.();
  ...
  return async (ctx, next) => {
    ...
    if (isAdmin(config, userId)) return next();
    ...
    if (entry.timestamps.length >= config.rateLimitMaxUpdates) {
      ...
      if (shouldNotify) logger.warn({ userId, updateType: ctx.updateType }, "throttled user");
      return;
    }
