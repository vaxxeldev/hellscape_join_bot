...
bot.on("text", async (ctx) => {
  ...
  await ctx.reply(
    "Я принимаю только анкеты на вступление и заявки на бронь ролей.\nВыберите действие ниже.",
    mainMenuKeyboard(getConfig()),
  );
});
...
async function launchWithRetry() {
  ...
  while (true) {
    try {
      ...
      logger.error(
        { error, retryInSeconds: getConfig().launchRetrySeconds, apiRoot: getConfig().telegramApiRoot },
        "failed to launch bot, retrying",
      );
